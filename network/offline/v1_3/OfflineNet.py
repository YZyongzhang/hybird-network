...
class SAC_Hybird_model(torch.nn.Module):
    ...
    def __init__(self,  state_dim, hidden_dim, action_dim, actor_lr, critic_lr,
        ...
        # v1_3: hybrid 与 SAC 联合训练
        self.hybird.train()
        self.hybird_optimizer = torch.optim.Adam(
            self.hybird.parameters(),
            lr=actor_lr / 10.0,
        )
        # 策略网络
    ...
    def _encode_state(self, audio, rgb, depth):
        return self.hybird.embedding_forward(
    ...
    def get_action(self, state):
        ...
        with torch.no_grad():
            state = self._encode_state(audio, rgb, depth)
    ...
    def update(self, states, actions, rewards, next_states, dones):
        ...
        states = self._encode_state(audio, rgb, depth)
        next_states = self._encode_state(audio_next, rgb_next, depth_next)
        states_actor = states.detach()
        ...
        # actor 只需要 q 的值，不应把梯度传到 critic 参数
