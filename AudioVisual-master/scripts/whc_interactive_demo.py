...
def interactive_demo(config, env, trainer):
    done = False
    ...
    while not done:
        ...
        _, action, _, test_recurrent_hidden_states, distributions = self.actor_critic.act(batch, test_recurrent_hidden_states, prev_actions, not_done_masks, deterministic=True)
    ...
    for img, aud in zip(frames, audios):
        ...
        if aud is None:
            ...
            set_trace()
    # set_trace()
    images_to_video_with_audio(frames, video_dir, video_name, audios, sr=config.TASK_CONFIG.SIMULATOR.AUDIO.RIR_SAMPLING_RATE, fps=fps)
...
def main():
    ...
    logging.basicConfig(level=level, handlers=[stdout_handler], format='%(asctime)s, %(levelname)s: %(message)s', datefmt="%Y-%m-%d %H:%M:%S")

    config = get_config(config_paths=args.exp_config, opts=args.opts, run_type=args.run_type)
    config.defrost()
    ...
    print('i got here!')
    if args.keys == '':
        interactive_demo(config, env, trainer)
    else:
