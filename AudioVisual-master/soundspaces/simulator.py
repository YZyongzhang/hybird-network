...
class SoundSpacesSim(Simulator, ABC):
    ...
    def action_space_shortest_path(self, source: AgentState, targets: List[AgentState], agent_id: int = 0) -> List[ShortestPathPoint]:
        pass
    ...
    def __init__(self, config: Config) -> None:
        ...
        self._action_space = spaces.Discrete(len(self.sim_config.agents[0].action_space))
        ...
        self._current_sound_id = 0
        self._sound_ids = np.load(self.config.AUDIO.SOURCE_SOUND_IDS_PATH, allow_pickle=True).item()
        self._sound_num = len(set(self._sound_ids.values()))
        print("sound num:", self._sound_num)
        self.DEPTH_NOISE_LEVEL = config.DEPTH_NOISE_LEVEL if hasattr(config, "DEPTH_NOISE_LEVEL") else None
        self.AUDIO_NOISE_LEVEL = config.AUDIO_NOISE_LEVEL if hasattr(config, "AUDIO_NOISE_LEVEL") else None
    ...
    def create_sim_config(self, _sensor_suite: SensorSuite) -> habitat_sim.Configuration:
        ...
        for sensor in _sensor_suite.sensors.values():
            ...
            # sim_sensor_cfg.parameters["hfov"] = str(170)
            # print(sensor.config.HFOV)
            # HFOV: 90
        ...
        agent_config.action_space = registry.get_action_space_configuration(self.config.ACTION_SPACE_CONFIG)(self.config).get()
    ...
    def current_scene_observation_file(self):
        return os.path.join(self.config.SCENE_OBSERVATION_DIR, self.config.SCENE_DATASET, self.current_scene_name + '.pkl')

    @property
    def current_source_sound(self):
        if self._current_sound not in self._source_sound_dict:
            audio_data, sr = librosa.load(os.path.join(self.source_sound_dir, self._current_sound), sr=self.config.AUDIO.RIR_SAMPLING_RATE)
            # self._source_sound_dict = dict()
    ...
    def reconfigure(self, config: Config) -> None:
        ...
        self._current_sound_id = self._sound_ids[self._current_sound.split('.')[0]]
        ...
        self._rotation_angle = int(np.around(np.rad2deg(quat_to_angle_axis(quat_from_coeffs(self.config.AGENT_0.START_ROTATION))[0]))) % 360
        if self.config.USE_RENDERED_OBSERVATIONS:
            self._sim.set_agent_state(list(self.graph.nodes[self._receiver_position_index]['point']), quat_from_coeffs(self.config.AGENT_0.START_ROTATION))
        else:
            self.set_agent_state(list(self.graph.nodes[self._receiver_position_index]['point']), self.config.AGENT_0.START_ROTATION)
        ...
        if self.config.AUDIO.HAS_DISTRACTOR_SOUND:
            ...
            self._distractor_position_index = np.random.choice(list(self.graph.nodes()))
            self._current_distractor_sound = np.random.choice(list(self._sound_ids.keys())) + ".wav"
            self._load_single_distractor_sound()

        if self._use_oracle_planner:
            self._oracle_actions = self.compute_oracle_actions()

        logging.debug("Initial source, agent at: {}, {}, orientation: {}".format(self._source_position_index, self._receiver_position_index, self.get_orientation()))
    ...
    def reset(self):
        ...
        # if self.DEPTH_NOISE_LEVEL is not None:
        #     observations["depth"] = observations["depth"] * (2 * self.DEPTH_NOISE_LEVEL * np.random.random(observations["depth"].shape) + (1 - self.DEPTH_NOISE_LEVEL))
    ...
    def step(self, action, only_allowed=True):
        ...
        """
        assert self._is_episode_active, ("episode is not active, environment not RESET or "
            ...
            if self.config.CONTINUOUS_VIEW_CHANGE:
                ...
                if action == HabitatSimActions.MOVE_FORWARD:
                    ...
                    for i in range(1, fps):
                        ...
                        self.set_agent_state(intermediate_position.tolist(), quat_from_angle_axis(np.deg2rad(self._rotation_angle), np.array([0, 1, 0])))
                        sim_obs = self._sim.get_sensor_observations()
                        observations = self._sensor_suite.get_observations(sim_obs)
                    ...
                    for i in range(1, fps):
                        if action == HabitatSimActions.TURN_LEFT:
                            ...
                            intermediate_rotation = prev_rotation_angle - i / fps * 90
                        self.set_agent_state(list(self.graph.nodes[self._receiver_position_index]['point']), quat_from_angle_axis(np.deg2rad(intermediate_rotation), np.array([0, 1, 0])))
                        sim_obs = self._sim.get_sensor_observations()
                        observations = self._sensor_suite.get_observations(sim_obs)
                        intermediate_observations.append(observations)

            self.set_agent_state(list(self.graph.nodes[self._receiver_position_index]['point']), quat_from_angle_axis(np.deg2rad(self._rotation_angle), np.array([0, 1, 0])))
        self._episode_step_count += 1

        # log debugging info
        logging.debug('After taking action {}, s,r: {}, {}, orientation: {}, location: {}'.format(action, self._source_position_index, self._receiver_position_index, self.get_orientation(), self.graph.nodes[self._receiver_position_index]['point']))
        ...
        # if self.DEPTH_NOISE_LEVEL is not None:
        #     observations["depth"] = observations["depth"] * \
        #                             (2 * self.DEPTH_NOISE_LEVEL * np.random.random(observations["depth"].shape)
        #                              + (1 - self.DEPTH_NOISE_LEVEL))
        return observations
    ...
    def _load_source_sounds(self):
        ...
        for sound_file in sound_files:
            ...
            sound = sound_file
            audio_data, sr = librosa.load(os.path.join(self.source_sound_dir, sound), sr=self.config.AUDIO.RIR_SAMPLING_RATE)
    ...
    def _load_single_distractor_sound(self):
        if self._current_distractor_sound not in self._source_sound_dict:
            audio_data, sr = librosa.load(os.path.join(self.distractor_sound_dir, self._current_distractor_sound), sr=self.config.AUDIO.RIR_SAMPLING_RATE)
    ...
    def _compute_audiogoal(self):
        ...
        if self._episode_step_count > self._duration:
            ...
            binaural_rir_file = os.path.join(self.binaural_rir_dir, str(self.azimuth_angle), '{}_{}.wav'.format(self._receiver_position_index, self._source_position_index))
            try:
            ...
            if self.current_source_sound.shape[0] == sampling_rate:
                binaural_convolved = np.array([fftconvolve(self.current_source_sound, binaural_rir[:, channel]) for channel in range(binaural_rir.shape[-1])])
                ...
                if index * sampling_rate - binaural_rir.shape[0] < 0:
                    ...
                    source_sound = self.current_source_sound[index * sampling_rate - binaural_rir.shape[0] + 1:(index + 1) * sampling_rate]
                    binaural_convolved = np.array([fftconvolve(
                        source_sound,
            ...
            if self.config.AUDIO.HAS_DISTRACTOR_SOUND:
                binaural_rir_file = os.path.join(self.binaural_rir_dir, str(self.azimuth_angle), '{}_{}.wav'.format(self._receiver_position_index, self._distractor_position_index))
                try:
                ...
                if self._current_sound not in self._source_sound_dict:
                    ...
                    self._source_sound_dict[self._current_sound] = audio_data
                self._audio_length = self._source_sound_dict[self._current_sound].shape[0] // self.config.AUDIO.RIR_SAMPLING_RATE
                distractor_convolved = np.array([fftconvolve(self._source_sound_dict[self._current_distractor_sound], distractor_rir[:, channel]) for channel in range(distractor_rir.shape[-1])])
                audiogoal += distractor_convolved[:, :sampling_rate]

            if self.AUDIO_NOISE_LEVEL is not None:
                audiogoal += np.random.randn(*audiogoal.shape) * self.AUDIO_NOISE_LEVEL * np.sqrt(np.sum(np.abs(audiogoal)))
    ...
    def get_current_spectrogram_observation(self, audiogoal2spectrogram):
        if self.config.AUDIO.HAS_DISTRACTOR_SOUND:
            ...
            if joint_index not in self._spectrogram_cache:
                audiogoal = self.get_current_audiogoal_observation()
                # if self.AUDIO_NOISE_LEVEL is not None:
                #     audiogoal = audiogoal * (2 * self.AUDIO_NOISE_LEVEL * np.random.random(audiogoal.shape) + (1 - self.AUDIO_NOISE_LEVEL))
    ...
    def get_oracle_action(self):
        if self._episode_step_count >= len(self._oracle_actions):
            return None
