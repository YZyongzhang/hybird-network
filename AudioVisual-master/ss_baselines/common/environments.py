...
class AudioNavRLEnv(habitat.RLEnv):
    ...
    def step(self, *args, **kwargs):
        # print(self._env.sim.forward_vector)
...
@baseline_registry.register_env(name="MyAudioNavRLEnv")
class MyAudioNavRLEnv(habitat.RLEnv):
    def __init__(self, config: Config, dataset: Optional[Dataset] = None):
        self._rl_config = config.RL
        self._core_env_config = config.TASK_CONFIG
        self._previous_target_distance = None
    ...
    def hack_observations(self, observations):
        ...
        observations['audiogoal'] = self._env.sim._compute_audiogoal()
        # # # set_trace()
...
@baseline_registry.register_env(name="WhcAudioNavRLEnv")
class WhcAudioNavRLEnv(habitat.RLEnv):
    def __init__(self, config: Config, dataset: Optional[Dataset] = None):
        self._rl_config = config.RL
        self._core_env_config = config.TASK_CONFIG
    ...
    def reset(self):
        ...
        observations = super().reset()
        logging.debug(super().current_episode)
        # print("audio===", self._env._sim.config.AGENT_0.SOUND_ID)
        # print(self._env._sim._current_sound)
        # if self.myconfig.feature_type == 1:
        #     self._env._sim._load_source_sounds()
        #     self.change_sound_and_get_spectrogram()
        # elif self.myconfig.feature_type == 2:
        #     if self.spects.keys is not None:
        #         df = pd.DataFrame([self.spects])
        #         with open('/data/AudioVisual/pkl/sp_.pkl', 'wb') as f:
        #             pickle.dump(df, f)
        #         self.spects = dict()

        # self.audio_use[self._env._sim.config.AGENT_0.SOUND_ID] = 1
        # print("len==", len(self.audio_use))
        self._previous_target_distance = self.habitat_env.current_episode.info["geodesic_distance"]
        info = {}
    ...
    def step(self, *args, **kwargs):
        ...
        # action = random.randint(0, len(self._env._sim.graph[self._env._sim._source_position_index]) + 1)
        # self.num_step += 1
        # self._env._sim._audiogoal_cache = dict()
        # if action == 0:
        #     pass
        # elif self.num_step % 3 == 0:
        #     prev_position_index = self._env._sim._source_position_index
        #     # the agent initially faces -Z by default
        #     n = 1
        #     for neighbor in self._env._sim.graph[self._env._sim._source_position_index]:
        #         if action == n:
        #             self._env._sim._source_position_index = neighbor
        #         else:
        #             n += 1
        # else:
        #     pass
        ...
        # self.spects[self.num_step] = observations
        # Don't know if we need new info or the past
...
@baseline_registry.register_env(name="DataGenerationRLEnv")
class DataGenerationRLEnv(habitat.RLEnv):
    def __init__(self, config: Config, dataset: Optional[Dataset] = None):
        self._rl_config = config.RL
        self._core_env_config = config.TASK_CONFIG
        self.myconfig = config
    ...
    def reset(self):
        if self.myconfig.feature_type == 0:
            self._env._sim._load_source_sounds()
            return
        self._previous_action = None
        self.num_step = 0
    ...
    def step(self, *args, **kwargs):
        nodes = list(self._env._sim.graph.nodes())
        widgets = ["Progress of env=%s:" % self._env._sim.current_scene_name, Percentage(), " ", Bar("#"), " ", Timer(), " ", ETA()]
        bar = ProgressBar(widgets=widgets, maxval=len(nodes))
        bar.start()
        a = []
        for i in range(len(nodes)):
            a.append(self._env._sim.graph.nodes[nodes[i]]['point'][1])
        bar.finish()
    ...
    def change_sound_and_get_spectrogram(self):
        if self.myconfig.feature_dir is not None:
            sound_names = os.listdir(self.myconfig.feature_dir)
        spectrograms = dict()
        ...
        for name in sound_names:
            ...
            spectrogram = self._env._sim.get_current_spectrogram_observation(self.compute_spectrogram)
            spectrograms[name] = spectrogram
            # print("Spectrograms at %d %d %s is saved" %(self._env._sim._source_position_index, self._env._sim._receiver_position_index, name))
        # df = pd.DataFrame([spectrograms])
        '''
        source_pos_idx = self._env._sim._source_position_index
        receiver_pos_idx = self._env._sim._receiver_position_index
        rotation = int(self._env._sim._rotation_angle / 90)
        data_dir = os.path.join("/data/AudioVisual/pre_train_data", self._env._sim.current_scene_name)
        if not os.path.isdir(data_dir):
            os.makedirs(data_dir)
        with open(os.path.join(data_dir, "{source_pos_idx}_{receiver_pos_idx}_{rotation}.pkl".format(**locals())), 'wb') as f:
            pickle.dump(df, f)
        '''

        d, r = self.get_dis_and_rot()
        # np.save(os.path.join(data_dir, "{source_pos_idx}_{receiver_pos_idx}_d_r.npy".format(**locals())),
        #         np.array([d, r]))
        return spectrograms, d, r
