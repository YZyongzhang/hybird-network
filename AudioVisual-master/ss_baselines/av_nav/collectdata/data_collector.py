...
from ss_baselines.common.rollout_storage import RolloutStorageAffordance, RolloutStorageNaive
...
SKIP_LIST = ['best_action', 'goals', 'episode_id', 'position', 'rotation', 'orientation',
             'graph', 'index', 'grid_size', 'scene_name', 'audiogoal']
...
@baseline_registry.register_trainer(name="AVDataCollector")
class AVDataCollector(BaseRLTrainer):
    ...
    def render(self, observations):
        for idx, obs in enumerate(observations):
            cv2.imshow(f'Env_{idx}', self.transform_rgb_bgr(observations[idx]["rgb"]))

    def _collect_rollout_step(
        self, rollouts, affordance_rollouts, current_episode_reward, current_episode_step, episode_rewards,
        ...
        # debug audio-diff
        # actions = torch.tensor([2]*self.envs.num_envs).unsqueeze(1)
        ...
        set_trace()
        # print(observations[0]['position'])
        # agent高度似乎是-1.5434817
        affordance_rollouts.update(observations, dones, [bool(info['success']) for info in infos], infos)
        # set_trace()
        # top_2_down_map = draw_top_down_map(infos[0])
    ...
    def train(self) -> None:
        ...
        exp_name = 'M8D26_fov130_2depth'
        # exp_name = 'M8D25_fov120'
        # exp_name = 'M8D25_fov120'
        # exp_name = 'M8D24_goal'
        # exp_name = 'M8D24_12grid'
        root_path = f'./Dataset/{exp_name}'
        exists_or_mkdir('./Dataset/')
        exists_or_mkdir(root_path)
        affordance_rollouts = RolloutStorageAffordance(
            self.envs.num_envs,
            self.envs,
            root_path,
            self.config,
        ...
        # set_trace()
        # 一开始的observations拿不到infos 导致拿不到地图
        # affordance_rollouts.update(observations, [False]*self.envs.num_envs, [False]*self.envs.num_envs)
        batch = batch_obs(observations, skip_list=SKIP_LIST)
        # expert_actions = None
