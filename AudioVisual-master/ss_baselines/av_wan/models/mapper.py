...
class Mapper(nn.Module):
    ...
    def update(self, prev_action: int, ego_map, intensity) -> Tuple[list, list]:
        ...
        if prev_action == HabitatSimActions.MOVE_FORWARD:
            ...
            pass
        #print(sum(sum(self._geometric_map)))
        # update global map
...
class WhcMapper(nn.Module):
    ...
    def update(self, prev_action: int, ego_map, intensity) -> Tuple[list, list]:
        ...
        # outputImg = Image.fromarray(self._geometric_map[:, :, 1] * 255.0)
        ...
        # outputImg = outputImg.convert('L')
        # outputImg.save(pt + "/" + self.time + "----" + str(self.debug_count) + 'a-a' + str(-self._rotation) + '.bmp')
        # update acoustic map
        if self._use_acoustic_map:
            am_x = self._x // self._stride
            am_y = self._y // self._stride
