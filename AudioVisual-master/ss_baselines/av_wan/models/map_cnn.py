...
class WhcMapCNN(nn.Module):
    ...
    def __init__(self, observation_space, output_size, map_type='gm', config=None):
        super().__init__()
        self._map_type = map_type
        self.config = config
        if map_type == 'am' or map_type == 'gm':
            self._n_input_gm = observation_space.spaces[map_type].shape[2]
            cnn_dims = np.array(observation_space.spaces[map_type].shape[:2], dtype=np.float32)
        ...
        if self._map_type == 'gm':
            ...
            # strides for different CNN layers
            self._cnn_layers_stride = [(2, 2), (1, 1), (1, 1)]
        elif self._map_type == 'cm':
            self._cnn_layers_kernel_size = [(3, 3), (3, 3), (3, 3)]
            # strides for different CNN layers
    ...
    def forward(self, observations):
        cnn_input = []
        if self._map_type in ['gm', 'am']:
            gm_observations = observations[self._map_type]
            # permute tensor to dimension [BATCH x CHANNEL x HEIGHT X WIDTH]
            gm_observations = gm_observations.permute(0, 3, 1, 2)
        else:
            gm_observations = observations.unsqueeze(dim=1)
        cnn_input.append(gm_observations)
