...
from ss_baselines.av_wan.models.mapper import Mapper, to_array, WhcMapper
...
class Planner:
    ...
    def __init__(self, task_config=None, use_acoustic_map=False, model_dir=None, masking=True):
        self.mapper = Mapper(gm_config=task_config.TASK.GEOMETRIC_MAP, am_config=task_config.TASK.ACOUSTIC_MAP, action_map_config=task_config.TASK.ACTION_MAP, use_acoustic_map=use_acoustic_map)
    ...
    def add_maps_to_observation(self, observation):
        ...
        if 'action_map' in observation:
            observation['action_map'] = np.expand_dims(self.mapper.get_egocentric_occupancy_map(size=self._action_map_size, action_map_res=self._action_map_res), -1).astype(np.float32)
    ...
    def plan(self, observation: dict, goal, stop, distribution=None) -> torch.Tensor:
        ...
        if stop:
            ...
            try:
                ...
                desired_orientation = np.round(np.rad2deg(np.arctan2(next_node_idx[1] - y, next_node_idx[0] - x))) % 360
                ...
                if adjacent_node in self._graph.nodes and (graph_nodes[0], adjacent_node) in self._graph.edges:
                    action = np.random.choice([HabitatSimActions.MOVE_FORWARD, HabitatSimActions.TURN_LEFT, HabitatSimActions.TURN_RIGHT])
                else:
                    action = np.random.choice([HabitatSimActions.TURN_LEFT, HabitatSimActions.TURN_RIGHT])
                self._prev_next_node = None
    ...
    def _map_to_graph(self, geometric_map: np.array) -> nx.Graph:
        ...
        occupancy_map = np.bitwise_and(geometric_map[:, :, 0] >= self._obstacle_threshold, geometric_map[:, :, 1] >= self._obstacle_threshold)
        graph = nx.Graph()
...
class WhcPlanner:
    ...
    def __init__(self, task_config=None, use_acoustic_map=False, model_dir=None, masking=True, config=None):
        self.mapper = WhcMapper(gm_config=task_config.TASK.GEOMETRIC_MAP, am_config=task_config.TASK.ACOUSTIC_MAP, action_map_config=task_config.TASK.ACTION_MAP, use_acoustic_map=use_acoustic_map, config=config)
        self.config = config
        self._action_map_res = task_config.TASK.ACTION_MAP.MAP_RESOLUTION
        self._action_map_size = task_config.TASK.ACTION_MAP.MAP_SIZE
        self._prev_depth = None
