...
class WhcVisualCNN(nn.Module):
    ...
    Args:
        ...
        output_size: The size of the embedding vector
    """
    def __init__(self, observation_space, output_size, encode_rgb, encode_depth, config=None):
        super().__init__()
        ...
        if self.is_blind:
            ...
            self.cnn = nn.Sequential(
                ...
                #  nn.ReLU(True),
                Flatten(),
                add_normalize(nn.Linear(32 * cnn_dims[0] * cnn_dims[1], output_size), config.normalize_config),
                nn.ReLU(True),
            )
        layer_init(self.cnn)
