...
from ss_baselines.av_wan.models.planner import Planner, WhcPlanner
...
class MapNavEnv(habitat.RLEnv):
    ...
    def step(self, *args, **kwargs):
        #kw["action"]是一个0:80的数，代表选9*9map当中的哪个点。
    ...
    def global_to_egocentric(self, pg):  #egocentric以自我为中心
...
@baseline_registry.register_env(name="WhcMapNavEnv")
class WhcMapNavEnv(habitat.RLEnv):
    ...
    def __init__(self, config: Config, dataset: Optional[Dataset] = None):
        ...
        self.planner = WhcPlanner(model_dir=self._config.MODEL_DIR, use_acoustic_map='ACOUSTIC_MAP' in config.TASK_CONFIG.TASK.SENSORS, masking=self._config.MASKING, task_config=config.TASK_CONFIG, config=config)
        torch.set_num_threads(1)
        self.num = 1
        self.debug_count = 0
    ...
    def reset(self):
        ...
        info['graph'] = self._env.sim.graph
        ...
        self.debug_count = 0
        self.time = time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime(time.time()))
        #self.waypoint_ids = torch.ones((self.planner._navigable_xs, self.planner._navigable_ys)) * 999999
        return observations
